...
Then user sees the cards sorted by market cap in descending order.
`;
describe(userStory, () => {
  ...
  it("shows the cards in the correct order", () => {
    ...
    expect(cards[0]).toContainElement(screen.getByText(/token 2/i));
    expect(cards[1]).toContainElement(screen.getByText(/token 3/i));
    expect(cards[2]).toContainElement(screen.getByText(/token 1/i));
  });
});
